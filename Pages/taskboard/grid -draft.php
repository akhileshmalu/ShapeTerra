...
Class Grid {
            ...
            foreach($row as $key=>$value) {
                ...
                if(!is_array($this->security) || in_array($key,$this->security)) {
                    ...
                    if(in_array($key,$_POST['saveable'])) {
                        ...
                        $value =  $value;
                        $setArray[] = "`$key`='$value'";
                        /* Orignal code below to stop sql injection
    ...
    function add() {
        ...
        if(empty($this->set)) {
            $pdo->exec("INSERT INTO {$this->table} VALUES ()");

            // if you passed a set param then use that in the insert
        } else {
            $pdo->exec("INSERT INTO {$this->table} SET {$this->set}");
        }
    ...
    function delete() {
        ...
        $primaryKey = $this->getPrimaryKey();
        return $pdo->exec("DELETE FROM {$this->table} WHERE `$primaryKey` = '$post[id]'");
    }
    ...
    function getPrimaryKey($table=NULL) {
        ...
        $smt = $pdo->prepare("SHOW KEYS FROM `$table` WHERE Key_name = 'PRIMARY'");

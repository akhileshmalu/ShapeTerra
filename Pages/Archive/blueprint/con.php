...
if (isset($_SERVER['HTTP_CAS_VIPID']) || isset($_SERVER['HTTP_CAS_UDC_IDENTIFIER']))  {

    $vipId = $_SERVER['HTTP_CAS_VIPID'];
    $uscid = $_SERVER['HTTP_CAS_UDC_IDENTIFIER'];

    $sql = isset($vipId)? "SELECT * FROM PermittedUsers WHERE VIP_ID =:vipid":
    "SELECT * FROM PermittedUsers WHERE NETWORK_USERNAME =:uscid ";

    //Query into database for record check
    //   $sql = "SELECT * FROM PermittedUsers WHERE VIP_ID =:vipid  ";

    $result = $connection->prepare($sql);                             //Query Execution
    $result->bindParam(':vipid', $vipId, 1);
    $result->execute();

    $row_cnt = $result->rowCount();                                 //count no of rows in result object.
    if ($row_cnt) {                                        //If there exist one or more records
...
//                session_start(); // Session Initiation
            ...
            //call via username
            if ($pArrayOfFields = $pClassUser->GetAttr($uscid, array("cn", "distinguishedName", "mail", "givenName", "sn")))
            {
                $lname = $pArrayOfFields["sn"];
                $email = $pArrayOfFields["mail"];
                $fname = $pArrayOfFields["givenName"];

             //   echo $lname . ", " . $fname . " " . $email . "<br />";
            }

            //call via vipid
            if ($pArrayOfFields = $pClassUser->GetUsernameByVIPID($vipId, array("uscEduUSCID", "distinguishedName", "givenName", "sn")))
            {
                $uscid = $pArrayOfFields["uscEduUSCID"];
                $lname = $pArrayOfFields["sn"];
                $fname = $pArrayOfFields["givenName"];

                echo $lname . ", " . $fname . " - " . $uscid . "<br />";
            }

            $_SESSION['login_vipId'] = $vipId;                   //session variable register
            header("location:account.php");                     //redirect to account page
            $_SESSION['isLogged'] = true;
        } else {
            if ($record['USER_STATUS'] == '0') {
                $message [0] = "Your account has been temporarily suspended. Please contact SysAdmin to activate it.";
            } // If User has been Terminated i.e. active = '-1'
            else {
                $message[0] = "Your Privileges have been terminated.";
            }

        }

    } else {
        $message[0] = "You don't have sufficient privileges to contribute in AcademicBlueprint System.";
    }
    //Close Connection
    $connection = null;
    $result = null;
